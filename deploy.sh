...
ssh $SERVER "
  ...
  if docker inspect $APP >/dev/null 2>&1; then
    docker inspect $APP --format '{{range .Config.Env}}{{println .}}{{end}}' \
      | grep -E '^(ANTHROPIC_API_KEY|ELEVENLABS_API_KEY|ELEVENLABS_VOICE_ID|DATABASE_URL|JWT_SECRET|STRIPE_SECRET_KEY|STRIPE_WEBHOOK_SECRET|STRIPE_PRICE_SOLO_PILOT|STRIPE_PRICE_CFI_PRO|STRIPE_PRICE_FLIGHT_SCHOOL|STRIPE_PRICE_SOLO_PILOT_ANNUAL|STRIPE_PRICE_CFI_PRO_ANNUAL|STRIPE_PRICE_FLIGHT_SCHOOL_ANNUAL|CRON_SECRET|RESEND_API_KEY|WEEKLY_FROM|APNS_KEY_ID|APNS_TEAM_ID|APNS_KEY_P8|APNS_BUNDLE|APNS_HOST|GLITCHTIP_DSN|APP_URL|PORT|HOSTNAME|ADMIN_KEY|WEEKLY_REPLY_TO)=' \
