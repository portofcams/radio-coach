...
export default function ScenarioPage() {
  ...
  const playTransmission = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        // TTS not configured — do nothing, text is already visible
        return
      }
  ...
  return (
    <main className="min-h-screen">
      ...
      <div className="max-w-2xl mx-auto px-6 py-10">
        ...
        {/* ATC Transmission — the radio display */}
                ...
                {result.elements.required.map((el) => {
                  ...
                  return (
                    <div key={el} className="flex items-center gap-2 text-sm">
                      <span
                        className={`shrink-0 ${hit ? 'text-green-600' : 'text-red-500'}`}
                      >
                        {hit ? '✓' : '✗'}
                      </span>
