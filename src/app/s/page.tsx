...
function ogUrl(sp: Record<string, string | string[] | undefined>): string {
  ...
  for (const k of ['score', 'passed', 'rate', 'label', 'cs']) {
    const v = sp[k]
...
export async function generateMetadata({ searchParams }: { searchParams: SP }): Promise<Metadata> {
  ...
  const title = score ? `${score}% radio-ready on Clearspar` : 'Clearspar Radio Trainer'
  const img = ogUrl(sp)
  return {
    title,
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-[#06070a] text-white px-6 py-16">
      <div className="max-w-md text-center">
        <div className="text-xs font-mono tracking-[0.25em] text-[#36d6e6] mb-5">CLEARSPAR · RADIO READINESS</div>
        {score ? <div className="text-8xl font-bold text-[#f5a623] leading-none mb-3">{score}%</div> : null}
        <div className="text-lg text-gray-200 mb-2">{label || 'Grade your ATC radio calls like a CFI.'}</div>
