...
// #98 -- publishes the currently-showing card so "Continue on iPhone" (the
// system Handoff affordance) can hand it to FlashcardsView over there. Both
// targets declare the SAME activity type in their Info.plist (a hard
// requirement for Handoff -- Apple won't offer it across apps otherwise).
private func drillActivity(kind: String, prompt: String) -> some ViewModifier {
    UserActivityPublisher(kind: kind, prompt: prompt)
}
private struct UserActivityPublisher: ViewModifier {
    let kind: String
    let prompt: String
    func body(content: Content) -> some View {
        content.userActivity("com.binnacleai.radiocoach.drill") { activity in
            activity.title = "Continue this drill"
            activity.userInfo = ["drillType": kind, "prompt": prompt]
...
struct PhoneticDrillView: View {
    ...
    var body: some View {
        Flashcard(prompt: String(ch), answer: answer) { ch = pool.randomElement()! }
            ...
            .modifier(drillActivity(kind: "phonetic", prompt: String(ch)))
    }
}
...
struct NumbersDrillView: View {
    ...
    var body: some View {
        Flashcard(prompt: item.display, answer: item.spoken) { item = Numbers.random() }
            ...
            .modifier(drillActivity(kind: "numbers", prompt: item.display))
    }
}
...
struct CallsignDrillView: View {
    ...
    var body: some View {
        Flashcard(prompt: cs, answer: Phonetics.say(cs)) { cs = Phonetics.randomCallsign() }
            ...
            .modifier(drillActivity(kind: "callsign", prompt: cs))
    }
}
