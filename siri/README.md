# Wilco — Siri App Shortcut

`Sources/WilcoShortcuts.swift` (committed) defines an App Intent + AppShortcutsProvider
so users can say **"Hey Siri, practice radio with Wilco"** (also "start a drill in
Wilco" / "quiz me on phonetics with Wilco") to open the app. App Shortcuts are
...
```sh
xcrun -sdk iphonesimulator swiftc -typecheck -target arm64-apple-ios16.0-simulator siri/Sources/WilcoShortcuts.swift
# (clean exit = OK; verified 2026-06-20, iOS 26.5 SDK)
```

## Notes
- The intent sets `openAppWhenRun = true` and opens Wilco to its home. Deep-linking
